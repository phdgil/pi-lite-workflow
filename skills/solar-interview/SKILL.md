...
If the tool is unavailable, explain that the complete pi-solar-lite package (skills plus runtime extension) must be installed, then pi restarted and the existing conversation reopened with `/resume`; do not fabricate a percentage or silently proceed without tracking. The user's session and previous answers must remain intact. `/solar-interview status` displays progress without an API request.
...
`/solar-interview review` rerates the existing saved evidence without creating a duplicate answer. The host retains the old assessment and labels the new result as a review. It does not automatically clear unresolved issues or force a particular score.
